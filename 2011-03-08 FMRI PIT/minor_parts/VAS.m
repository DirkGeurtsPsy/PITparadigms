...
barcol      = [255 200 0];
slider      = [0 0 10 50];
stepsize    = 8; %stepsize of the slider
...
for s=1:2
    ...
    if dutch==1
        text{1}={'Je krijgt nu het bittere sapje driemaal toegediend',...
            '(met korte tussenpozen)','Proef het goed.'};
        text{2}={'Je krijgt nu het zoete sapje driemaal toegediend',...
            '(met korte tussenpozen)','Proef het goed.'};
    elseif english==1
        text{1}={'You will now receive the bitter juice three times',...
            '(with short breaks in between).','Taste it well.'};
        text{2}={'You will now receive the sweet juice three times',...
            '(with short breaks in between).','Taste it well.'};
    end
    ...
    while true
        ...
        elseif english==1
            txt1={'Move the cursor with the left (index finger) and right (middle finger)',...
                'buttons to the position that you think fits the taste of the juice.',...
                'Then press the confirmation button (ring finger)'};
            txt2={'Very bad','Neutral','Very nice'};
            txt3 = {'press again to confirm, or press left or right to continue setting the slider'};
        end
            nrow=length(txt1);
        for k=1:nrow
    ...
    OVas(vasn)=(xpos-0.1*wdw)/(0.9*wdw-0.1*wdw); %sla percentage van balk op in vector.
