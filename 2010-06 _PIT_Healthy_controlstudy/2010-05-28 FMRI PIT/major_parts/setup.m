...
[wd, windowRect] = Screen('OpenWindow', 0, [0 0 0], [100 100 1100 1100]);
[widthwindowRect,heightwindowRect] = RectSize(windowRect);
end
...
for R=1:4; 
    [imagename map alpha] = imread(eval(['''shrooms/trainingmshr_' num2str(R) '.png'''])); 
    imagename(:,:,4) = alpha(:,:); 
...
for R=1:4; 
    [imagename map alpha] = imread(eval(['''shrooms/trainingshll_' num2str(R) '.png'''])); 
    imagename(:,:,4) = alpha(:,:); 
    Screen('BlendFunction', wd, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); 
...
for R=1:St.Inst.N; 
    [imagename map alpha] = imread(eval(['''shrooms/shroom_' num2str(R) '.png'''])); 
    imagename(:,:,4) = alpha(:,:); 
    Screen('BlendFunction', wd, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); 
...
for R=Ap(1,1:St.Pav.N); 
    eval(['pic=imread(''stimuli/fractal' num2str(R) '.jpg'');'])
...
for R=1:St.Inst.N; 
    [imagename map alpha] = imread(eval(['''shrooms/shell_' num2str(R) '.png'''])); 
    imagename(:,:,4) = alpha(:,:); 
    Screen('BlendFunction', wd, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); 
